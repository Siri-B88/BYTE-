...
def render_overview_card(city):
    ...
    cols = st.columns(2)
    cols[0].metric("🌡️ Temp", data['temperature'])
...
def metrics_heat_map(data):
    st.metric("🌡️ Avg. Surface Temperature", data['avg_temp'])
    st.info(f"🛰️ Data Source: {data['data_source']}")
...
def render_simulator():
    st.header("🛠️ Urban Planning Simulator")
    st.warning("Data on this page is simulated.")
    city = st.session_state.get("city", "London")
